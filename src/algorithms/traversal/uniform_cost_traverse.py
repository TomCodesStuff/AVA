...
class UniformCostTraverse(Algorithm[Graph]):
    ...
    def __selectNode(self, frontier : dict) -> Tuple[Node,int]: 
        selected_node = None 
        shortest_distance = float("inf")

        for (node, distance) in frontier.items(): 
    ...
    def run(self) -> int:  
        ...
        while (frontier and not goalFound): 
            # Reset all colours 
            graph.resetColours()
            
            # Get 
            crrntNode, crrntCost = self.__selectNode(frontier)
            ...
            for (neighbour, weight) in crrntNode.getNeighbours(): 
                ...
                if neighbour not in visitedNodes and totalCost < frontier.get(neighbour, float("inf")): 
                    frontier[neighbour] = totalCost
            ...
            visitedNodes[crrntNode] = crrntCost 
            # Remove node from frontier 
            del frontier[crrntNode]
