...
if TYPE_CHECKING:
    from src.screens.array_search import SearchController, SearchModel 
...
class SearchScreen(ArrayAlgorithmScreen[C, M, D]): 
    ...
    def __createTargetAdjuster(self) -> None:
        ...
        self.__targetSlider = tk.Scale(self.getOptionsWidgetFrame(), from_ = 0, to_ = 2, 
                                       length = self.getOptionsWidgetFrame().winfo_width(), orient = "horizontal", 
                                       bg = "white", highlightbackground = "white", 
                                       showvalue = False, command = self.updateDisplayedText)
